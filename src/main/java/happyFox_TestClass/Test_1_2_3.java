...
public class Test_1_2_3 extends driverClass {
	...
	{
		TT.ticketDetail(file.getProperty("TicketSub"), file.getProperty("TicketMess"));
		TT.contactDetails(file.getProperty("TicketName"), file.getProperty("TicketEmail"));
		TT.clickCreate();
	}
	@Test(priority=5)
	void ticketVerification() throws InterruptedException
	{
		...
		TTCreated.TTReply(file.getProperty("TicketReply"));
		TTCreated.changingTTStatus(file.getProperty("TicketStatus"), file.getProperty("TicketPriority"));
	...
	void statusDelete() throws InterruptedException
	{
		...
		delStatus.clickDel();
		delStatus.delReason(file.getProperty("TicketPriority"));
